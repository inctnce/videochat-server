...
class UserRepo extends Repo implements IRepo {
  ...
  async update(id: string, value: string | null, field: string): Promise<QueryResult> {
    let query: string = this.setUpdateQuery(value, field, id);
  ...
  async doesExist(nickname?: string, email?: string): Promise<boolean> {
    let nicknameExists = nickname ? (await this.readOne(nickname, "nickname")).user : undefined;
    let emailExists = email ? (await this.readOne(email, "email")).user : undefined;
